...
fs.readFile("server.js", function (err, data) {
    if (err) throw err;
    if (data.includes("res.end(\"Junior Frontend Vizsga\");")) {
...
fs.readFile("server.js", function (err, data) {
    if (err) throw err;
    if (data.includes("console.log(`A vizsga szerver a http://${hostname}:${port}/ címen elérhető`);") || data.includes("console.log(\"A vizsga szerver a http://127.0.0.1:80803000/  címen elérhető\");")) {
